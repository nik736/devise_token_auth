...
module DeviseTokenAuth::Concerns::SetUserByToken
  ...
  def set_user_by_token(mapping = nil)
    ...
    if request.headers["Authorization"].present?
      ...
      decoded_header = Base64.urlsafe_decode64(request.headers["Authorization"])
      req_headers = decoded_header.split(",")
    ...
    elsif request.headers["access-token"].present?

      # parse header for values necessary for authentication
      uid = request.headers["uid"]
      @token = DeviseTokenAuth::TokenFactory.new unless @token
      @token.token = request.headers["access-token"]
      @token.client = request.headers["client"]
